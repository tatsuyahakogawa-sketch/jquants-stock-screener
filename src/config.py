...
# PBR(株価純資産倍率)が割安とみなす上限（この値以下を「低PBR」とする）
PBR_LOW_THRESHOLD = 1.0
...
# メイン画面の期間指定検索で、TDnet開示件数がこれを超えた場合、新工場・新店舗・
# 東証移籍・株式分割・プライム市場変更・大型受注・世界初の発表（タイトルの
# キーワード一致で判定するTDNET_TITLE_BASED_RULES）の検索を行わない。期間が
# 広すぎる状態でタイトル検索を続けると結果が膨大になり実用的でないため、
# 期間を絞り込むよう促す（2026-08-24にユーザーが指定）。
MAX_TDNET_DISCLOSURES_FOR_SCREENING = 200

# 地方単独上場企業の決算短信XBRL取得をどこまで遡って試みるか。
# TDnetの開示添付ファイルは公開から約1〜1.5ヶ月で取得できなくなる
# (2026-08-19に実機確認: 公開41日後は404、36日後は200)。それより古い開示に
# ついてXBRL取得を試みても無駄にHTTPリクエストが増えるだけのため、
# REGIONAL_LISTING_LOOKBACK_YEARS(新規上場・大型イベント検出用)とは別に、
# 決算短信の財務データ取得だけこの日数に絞る（余裕を見て60日）。
REGIONAL_STATEMENTS_LOOKBACK_DAYS = 60
