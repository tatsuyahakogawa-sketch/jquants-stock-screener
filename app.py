"""日本株ツールのStreamlit UI（スクリーニング / 地方株の2モードを1画面内で切り替え）。
...
st.set_page_config(page_title="日本株ツール", layout="wide")
...
def render_screening_page() -> None:
    st.subheader("スクリーニング（須田忠雄事務所ルール）")
...
def render_regional_stocks_page() -> None:
    ...
    """
    st.subheader("地方株 - 地方上場株・重要イベント検索")
    st.caption(
