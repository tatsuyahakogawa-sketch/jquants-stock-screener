...
- 基本フロー: Claude Codeが実装 → GitHubへ反映 → Codexにレビュー依頼 → 指摘を確認 → Claude Codeが修正 → 動作確認 → GitHubへ反映
...
**Codexへのレビュー依頼タイミング**: ある程度まとまった実装（新機能・ロジック変更等）が完了し動作確認も済んだタイミングで、Claude Code側から「ここでCodexにレビューさせた方がよい」とユーザーに提案し、Codexにそのまま貼り付けられるレビュー依頼用プロンプトを作成する。
