...
- `scripts/watch_and_notify.py`: ストップ高・株式分割/併合・経常利益急増（前年同期比+50%以上）・東証新規上場（承認発表・上場前日リマインダー）を平日10:00/13:00/15:30 JSTに定期チェックしDiscordへ通知するバッチ（2026-08-27にユーザー指定、README「Discord通知」参照）。GitHub Actions(`.github/workflows/watch_and_notify.yml`)から実行され、`app.py`のUI安全弁（TDnet開示件数上限）を経由せずrules.py/endpoints.pyを直接呼ぶ。通知済み状態・各データ源のウォーターマーク（前回成功した走査開始日。長期休場明けの取りこぼし防止用）は`data/notify_state.json`にJSONで保存し、ワークフロー側で`notify-state`という専用ブランチにコミットして実行間（使い捨てコンテナ）をまたいで永続化する（`main`にコミットするとStreamlit Cloudの不要な再デプロイを招くため専用ブランチに分離。2026-08-27のCodexレビューで指摘・修正）。15:30 JST枠は大引け後のストップ高を同日中に通知するため2026-09-17に追加（`_stop_high_candidates`は当日を含めて`get_daily_quotes_range`を呼ぶ。以前は当日を除外していた）。新規上場は以前「本日上場」を通知していたが、当日では既に取引が始まっており手遅れだとユーザーに指摘され、2026-09-17に`detect_listings_tomorrow`（上場日が今日の翌日と完全一致する銘柄のみ）へ変更した。他のruleと異なり意図的にwatermarkによるcatch-upを行わない（一時的な失敗で前日に送れなかった場合、後から送っても手遅れで無意味なため、ユーザーが「送らない方がよい」と明言した）
