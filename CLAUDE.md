...
- `app.py`: Streamlit UI。画面上部の「表示するページ」（スクリーニング/地方株）で切り替える単一ページ構成（2026-08-19に`pages/`のマルチページ構成から統合）。スクリーニング: 絞り込み条件選択 → スクリーニング実行 → 銘柄集計テーブル/CSV。地方株: 下記`src/regional_stocks.py`のUI
