...
- `src/endpoints.py`: 日付単位バルク取得 + ローカルキャッシュ（`data/cache/`, parquet）。`get_daily_quotes_by_date`は日付が今日以降の間はキャッシュを使わず毎回APIから取得し直す（株価四本値は確定時刻が公式に案内されておらず、`get_statements_by_date`のような固定時刻での場合分けができないため。日付が過去になった時点で初めて恒久キーを使う。2026-09-17にストップ高の当日中通知に対応する際に導入）
...
- `scripts/watch_and_notify.py`: ストップ高・株式分割/併合・経常利益急増（前年同期比+50%以上）・東証新規上場（承認発表・上場前日リマインダー）を平日10:00/13:00/16:10 JSTに定期チェックしDiscordへ通知するバッチ（2026-08-27にユーザー指定、README「Discord通知」参照）。GitHub Actions(`.github/workflows/watch_and_notify.yml`)から実行され、`app.py`のUI安全弁（TDnet開示件数上限）を経由せずrules.py/endpoints.pyを直接呼ぶ。通知済み状態・各データ源のウォーターマーク（前回成功した走査開始日。長期休場明けの取りこぼし防止用）は`data/notify_state.json`にJSONで保存し、ワークフロー側で`notify-state`という専用ブランチにコミットして実行間（使い捨てコンテナ）をまたいで永続化する（`main`にコミットするとStreamlit Cloudの不要な再デプロイを招くため専用ブランチに分離。2026-08-27のCodexレビューで指摘・修正）。16:10 JST枠は大引け(15:30 JST。2024年11月の取引時間延長後の現行時刻。以前15:00と誤記していた点を2026-09-17のCodexレビューで指摘・修正)後のストップ高を同日中に通知するため2026-09-17に追加（`_stop_high_candidates`は当日を含めて`get_daily_quotes_range`を呼ぶ。以前は当日を除外していた）。`get_daily_quotes_by_date`のキャッシュ名前空間を`daily_quotes`から`daily_quotes_v2`に変更（`app.py`の対話的な画面が終了日デフォルト=本日で大引け前に当日分を素通しで要求すると、旧コードでは空/不完全なレスポンスが恒久キーで保存されてしまっていた可能性があり、その汚染エントリを無効化するため。同2026-09-17のCodexレビューで指摘・修正）。新規上場は以前「本日上場」を通知していたが、当日では既に取引が始まっており手遅れだとユーザーに指摘され、2026-09-17に`detect_listings_tomorrow`（次の営業日が上場日と一致する銘柄のみ。土日・祝日を挟む場合は`src/market_calendar.is_market_holiday`で次の営業日を計算するため、月曜上場・連休明け上場でも前営業日にリマインダーが届く。2026-09-17のCodexレビューで「土日・祝日を挟むと`today+1日`が非営業日になり、その日はワークフロー自体が動かないため永久に検出できない」と指摘され、単純な`today+1日`から修正）へ変更した。他のruleと異なり意図的にwatermarkによるcatch-upを行わない（一時的な失敗で前日に送れなかった場合、後から送っても手遅れで無意味なため、ユーザーが「送らない方がよい」と明言した）
