...
st.caption(
    ...
    "大型企業イベントをTDnet開示から検出します。時価総額は福証単独上場企業のみ"
    "yfinanceで取得できる現在値の近似値で、それ以外は取得不可（空欄）です。"
)
...
listed_only = listing_events.loc[listing_events["Stage"] == "上場"]
listing_date_by_code = (
    listed_only.groupby("Code")["Date"].max() if not listed_only.empty else pd.Series(dtype="datetime64[ns]")
)

market_cap_by_code = company_status.set_index("Code")[["MarketCap", "MarketCapNote"]] if not company_status.empty else None


def _market_cap_display(code: str) -> str:
    if market_cap_by_code is None or code not in market_cap_by_code.index:
        return "―"
    row = market_cap_by_code.loc[code]
    if pd.notna(row["MarketCap"]):
        return f"{row['MarketCap']:,.0f}円（現在値のみ、株数未反映）"
    return str(row["MarketCapNote"]) if row["MarketCapNote"] else "取得不可"


def _listing_date_display(code: str) -> str:
    if code not in listing_date_by_code.index:
        return "不明"
    return listing_date_by_code.loc[code].strftime("%Y-%m-%d")
...
for _, r in listing_events.iterrows():
    ...
    rows.append({
        ...
        "コード": r["Code"],
        "会社名": r["CompanyName"],
        "市場": "・".join(regional_stocks.regional_markets_in(r["MarketsString"])) or r["MarketsString"],
        "上場日": _listing_date_display(r["Code"]),
        "時価総額": _market_cap_display(r["Code"]),
        "重要イベント": label,
        "発表日": r["Date"],
        "内容": r["Title"],
        "URL": r["Url"],
...
if events_df.empty:
    st.info("検出されたイベントはまだありません。")
    st.stop()

sort_option = st.radio("並び順", ["イベントの新しい順", "上場日の新しい順", "最重要イベントを先頭に"], horizontal=True)
if sort_option == "イベントの新しい順":
    events_df = events_df.sort_values("発表日", ascending=False)
elif sort_option == "上場日の新しい順":
    events_df = events_df.sort_values("上場日", ascending=False)
else:
    events_df = events_df.sort_values(["_is_tokyo", "発表日"], ascending=[False, False])

st.dataframe(
    events_df.drop(columns=["id", "_is_tokyo"]),
